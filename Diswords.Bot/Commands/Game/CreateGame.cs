...
{
    [Group("game")]
    public class GameControls : BaseCommandModule
    {
        [Command("create")]
        public async Task CreateGameCommand(CommandContext ctx)
        {
            ...
            switch (roomOrChannel)
            {
                case "room":
                {
                    ...
                    var channel =
                        await ctx.Guild.CreateChannelAsync(id, ChannelType.Text, category, "Playing Diswords!");
                    ...
                    var databaseGame = new DatabaseGame(id, language, "", ctx.User.Id.ToString(), (int) gameType.Type,
                        (long) ctx.User.Id, (long) ctx.Guild.Id, (long) channel.Id);
                ...
                case "channel":
                {
                    ...
                    await message.ModifyAsync(EmbedHelper.SimpleEmbed(creatingGame));
                    var databaseGame = new DatabaseGame(id, language, "", ctx.User.Id.ToString(), (int) gameType.Type,
                        (long) ctx.User.Id, (long) ctx.Guild.Id, (long) ctx.Channel.Id);
                    GameDatabaseHelper.InsertGame(databaseGame);
        ...
        private static async Task<string?> GetRoomOrChannel(Locale locale, CommandContext ctx)
        {
            ...
            builder.AddComponents(new DiscordButtonComponent(ButtonStyle.Primary, "room", room),
                new DiscordButtonComponent(ButtonStyle.Primary, "channel", channel));
        ...
        private static async Task<GameInfo?> GetGameType(Locale locale, CommandContext ctx)
        {
            return await WaitForDropdown(locale, ctx, "SelectGameType", () => GetGameTypeDropdown(locale),
        ...
        {
            return await WaitForDropdown(locale, ctx, "SelectLanguage", GetLanguageDropdown, s => s);
        }

        private static async Task<T?> WaitForDropdown<T>(Locale locale, CommandContext ctx, string placeholder,
            Func<IEnumerable<DiscordSelectComponentOption>> elements, Func<string, T> returnValue) where T : class
        {
        ...
        private static IEnumerable<DiscordSelectComponentOption> GetGameTypeDropdown(Locale locale)
        {
            return GameSettings.GameTypes.Values.Select(info =>
                new DiscordSelectComponentOption($"{locale[info.LocaleName]}", info.ShortName,
                    emoji: new DiscordComponentEmoji(DiscordEmoji.FromUnicode(info.Emoji)))).ToList();
        }
