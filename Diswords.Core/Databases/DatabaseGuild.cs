...
namespace Diswords.Core.Databases
{
    ...
    public static class GuildDatabaseHelper
    {
        public static DatabaseGuild GetGuild(ulong id)
        {
            ...
            reader.Read();
            var guild = new DatabaseGuild((ulong) reader.GetInt64(0), (ulong) reader.GetInt64(1),
                (uint) reader.GetInt32(2), reader.GetString(3));
            reader.Close();
            return guild;
        }
        ...
        public static IEnumerable<DatabaseGuild> GetAllGuilds()
        {
            ...
            while (reader.Read())
                list.Add(new DatabaseGuild((ulong) reader.GetInt64(0), (ulong) reader.GetInt64(1),
                    (uint) reader.GetInt32(2), reader.GetString(3)));
            reader.Close();
            return list;
        }
        ...
        public static void InsertGuild(DatabaseGuild guild)
        {
            DatabaseHelper.ExecuteNonQuery(
                $"replace into guilds values({guild.Id}, {guild.GamesPlayed}, {guild.ParentGameCategory}, '{guild.Language}')");
        }

        public static string GetLanguage(ulong id)
        {
            return (string) GuildScalar(id, $"select language from guilds where id == {id}");
        }

        public static void SetLanguage(ulong id, string language)
        {
            GuildNonQuery(id, $"update guilds set language = '{language}' where guilds.id == {id}");
        }
        ...
        {
            var exists =
                (long) DatabaseHelper.ExecuteScalar($"select exists(select 1 from guilds where guilds.id == {id})");
            if (exists != 0) return;
